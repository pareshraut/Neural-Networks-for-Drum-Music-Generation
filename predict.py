...
def generate(tempo=120, length=2000,output_file = 'predicted.mid'):
    """ Generates the midi file """
    #load the notes used to train the model
    with open('data/full_notes', 'rb') as filepath:
    ...
    prediction_output = generate_notes(model, network_input, pitchnames,
                                       n_vocab, length)
    midi_file = create_midi(prediction_output, tempo)
    # Save the MIDI file
...
def create_network(network_input, n_vocab):
    ...
    model.load_weights('weights/weights-improvement-09-0.9702-bigger.hdf5.hdf5')
